...
# input
g = (basis(N, 0)+basis(N, 7))/math.sqrt(2)
w = (basis(N, 1)+basis(N, 2)+basis(N, 4))/math.sqrt(3)

rho1 = g*g.dag()
rho2 = w*w.dag()

p_error = (1-(rho1-rho2).norm()/2)/2

print('p_error=%s' % p_error)

input_dm_list = [rho1, rho2]
...
for point in range(0, 2):
    result.append(tot_solve(point, parameters_list))
