...
sample = []
for m in range(M):
    theta_m = 2*math.pi*(m/M)
    sample.append(pure(theta_m, 0))
sample_list = [sample]
# print(sample)
...
for i in range(M):
    desired_output_dm_set.append(basis(N, N-M+i)*basis(N, N-M+i).dag())
...
def tot_solve(point_data, param, s):
    ...
    return vlplc._gradient(input_dm, desired_output_dm), 1-((output_dm * desired_output_dm).tr()), list(ctrl.get_parameters()[0])
...
def evolution_for_sample(s):
    ...
    while update_num < update_tot:
        ...
        if update_num == update_tot-1:
            for i in result:
                every_state_final.append(i[1])
                final_params = i[2]
    return cost_list, every_state_final, final_params


if __name__ == "__main__":
    result_sample = []
    pool = multiprocessing.Pool(processes=state_pair_num)
    for sam in range(0, state_pair_num):
        result_sample.append(pool.apply_async(evolution_for_sample, (sam, )))
    pool.close()
    pool.join()

    every_state_final_cost = []
    cost_list_sample = []
    final_params_sample = []
    for i in result_sample:
        cost_list_sample.append(i.get()[0])
        every_state_final_cost.append(i.get()[1])
        final_params_sample.append(i.get()[2])
    print('cost_list_sample=%s' % cost_list_sample)
    print("final_params_sample={}".format(final_params_sample))
    print("every_state_final_cost={}".format(every_state_final_cost))
