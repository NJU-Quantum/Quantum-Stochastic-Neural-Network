...
def pure(theta, phi):
    psi = math.cos(theta/2)*basis(N, 0) + cmath.exp(1j*phi)*math.sin(theta/2)*basis(N, 1)
    rho = psi*psi.dag()
    return rho
...
theta_list = []
phi1 = 0
for phi2 in np.arange(0, 2*math.pi, math.pi/6):
    sample = [pure(math.pi/2, phi1), pure(math.pi/2, phi2)]
